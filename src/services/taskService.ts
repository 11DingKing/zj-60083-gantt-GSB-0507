...
export async function createTask(
  ...
  const siblingTasks = await db.getAllFromIndex(
    "tasks",
    "by-parentId",
    parentId,
  );
  ...
  await db.add("tasks", JSON.parse(JSON.stringify(task)));
...
export async function updateTask(
  ...
  await db.put("tasks", JSON.parse(JSON.stringify(updatedTask)));
...
async function updateParentDates(
  projectId: string,
  ...
  const duration =
    Math.ceil((maxEnd.getTime() - minStart.getTime()) / (24 * 60 * 60 * 1000)) +
    1;
  ...
  await db.put("tasks", JSON.parse(JSON.stringify(updatedParent)));
...
async function updateChildTasksStatus(
  projectId: string,
  parentId: string,
): Promise<void> {
  const db = await getDB();
...
export async function deleteTask(taskId: string): Promise<Task[]> {
  ...
  const deps = await tx
    .objectStore("dependencies")
    .index("by-projectId")
    .getAll(task.projectId);
...
async function deleteTaskRecursive(
  tx: IDBTransaction,
  taskId: string,
): Promise<void> {
