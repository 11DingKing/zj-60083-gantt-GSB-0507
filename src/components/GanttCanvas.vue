...
function drawTaskBar(
  ...
  if (task.progress > 0 && task.progress < 100) {
    ...
    ctx.globalAlpha = 0.4;
    ctx.beginPath();
    ctx.roundRect(x + progressWidth, y, width - progressWidth, TASK_HEIGHT, [
      0,
      radius,
      radius,
      0,
...
function drawBaselines(
  ...
  visibleTasks.value.forEach((task, index) => {
    ...
    if (
      startX !== dateToX(new Date(task.startDate)) - scrollX ||
      taskWidth !==
...
function drawDependencies(
  ...
  dependencies.forEach((dep) => {
    const fromIndex = visibleTasks.value.findIndex(
      (t) => t.id === dep.fromTaskId,
    );
...
function drawDependencyPreview(
  ctx: CanvasRenderingContext2D,
  scrollX: number,
  scrollY: number,
): void {
  ...
  const fromIndex = visibleTasks.value.findIndex(
    (t) => t.id === dependencyFromTaskId.value,
  );
  ...
  const currentX =
    dependencyStartX.value + (dragStartX.value - store.scrollLeft) - scrollX;
...
function handleMouseMove(e: MouseEvent): void {
  ...
  if (dragType.value === "progress") {
    ...
    const newProgress = Math.max(
      0,
      Math.min(100, Math.round(dragStartProgress.value + progressDelta)),
    );
  ...
  if (dragType.value === "move") {
    if (deltaDays !== 0) {
      const newStart = new Date(
        dragStartDate.value!.getTime() + deltaDays * 24 * 60 * 60 * 1000,
      );
      const newEnd = new Date(
        newStart.getTime() + (task.duration - 1) * 24 * 60 * 60 * 1000,
      );
    ...
    if (newStart < originalEnd) {
      const newDuration =
        Math.ceil(
          (originalEnd.getTime() - newStart.getTime()) / (24 * 60 * 60 * 1000),
        ) + 1;
    ...
    const newEnd = new Date(
      originalStart.getTime() +
        (task.duration - 1 + deltaDays) * 24 * 60 * 60 * 1000,
    );

    if (newEnd > originalStart) {
      const newDuration =
        Math.ceil(
          (newEnd.getTime() - originalStart.getTime()) / (24 * 60 * 60 * 1000),
        ) + 1;
...
watch(
  [
    ...
    zoom,
    () => store.dependencies,
    () => store.selectedBaseline,
    () => store.filterByAssigneeId,
  ],
  () => {
    nextTick(() => {
      updateCanvasSize();
